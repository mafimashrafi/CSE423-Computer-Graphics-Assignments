...
def check_enemy_in_line_of_sight():
    ...
    # Gun barrel position (end of the barrel)
    ...
    for enemy in enemie:
        ...
        if dist > 0:  # Avoid division by zero
            ...
            dot_product = gun_dir_x * dx_norm + gun_dir_y * dy_norm
            
            # If dot product is close to 1, enemy is in line of sight
            # Using a threshold for some tolerance (0.95 means very close alignment)
            if dot_product > 0.95 and dist < 800:  # Also check distance limit
                return True
...
def reset_game():
    ...
    global pause, shoot_bullet, game_over, auto_rotation, auto_shoot
...
def animation():
    ...
    # Auto shoot when enemy is in line of sight
    ...
    if shoot_bullet and single_bullet is None:
        ...
        # Reset shoot_bullet if it was triggered by auto_shoot
        if auto_shoot:
            shoot_bullet = False
